...
class GenderDetectionPipeline(BasePipeline):
    name = "GENDER"
    ...
    def process(self, face_infos):
        ...
        for cam_id, value in face_infos.items():
            ...
            if not getattr(value["services"], GenderDetectionPipeline.name):
                continue
            ...
            for frame_index, info in enumerate(facial_info):
                for order_index, detail in enumerate(info):
                    ...
                    gender_status = detail.get(GenderDetectionPipeline.name)
